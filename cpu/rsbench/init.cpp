...
SimulationData initialize_simulation(Input input) {
  ...
  SD.d_poles =
      (Pole *)calloc(input.n_nuclides * SD.max_num_poles, sizeof(Pole));
  SD.length_poles = input.n_nuclides * SD.max_num_poles;

  // Prepare full Window grid
  printf("Generating window parameter grid...\n");
  SD.windows = generate_window_params(input, SD.n_windows, SD.n_poles, &seed,
                                      &SD.max_num_windows);
...
Pole *generate_poles(Input input, int *n_poles, uint64_t *seed,
  ...
  *max_num_poles = max_poles;
  ...
  for( int i = 0; i < input.n_nuclides; i++ )
          for( int j = 0; j < n_poles[i]; j++ )
                  printf("R[%d][%d]: Eo = %lf lambda_o = %lf Tn = %lf Tg = %lf
  Tf = %lf\n", i, j, R[i * max_poles + j].Eo, R[i * max_poles + j].lambda_o, R[i
  * max_poles + j].Tn, R[i * max_poles + j].Tg, R[i * max_poles + j].Tf);
  */
